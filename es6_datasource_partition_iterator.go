...
func (espi *es6PartitionIterator) NextPartition() (sif.Partition, func(), error) {
	espi.lock.Lock()
	defer espi.lock.Unlock()
...
func (espi *es6PartitionIterator) producePartition(res *es6api.Response) (sif.Partition, func(), error) {
	...
	// produce partition
	part := datasource.CreateBuildablePartition(espi.source.conf.PartitionSize, espi.widestInitialSchema)
	if len(hits) < 1 {
		// close scroll
